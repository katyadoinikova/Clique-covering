...
first_val = LpVariable.dicts("first_val", range(10, num_v), cat="Integer")

for k in range(10, num_v):

    model += first_val[k] == lpSum(i * vert[i][k]  for i in range(3))


for k in range(10, num_v - 1):
...
u = LpVariable.dicts("u", (range(2, n-1), range(num_v)), cat="Binary")  # u[i][k] = 1 ⇔ все позиции до k-1 равны
gt = LpVariable.dicts("gt", (range(2, n-1), range(num_v)), cat="Binary")  # gt[i][k] = 1 ⇔ x[3*i][k]=1, x[3*(i+1)][k]=0 и до k все равны
...
for i in range(2, n - 1):
    model += lpSum(gt[i][k] for k in range(num_v)) >= 1
...
# eq1 = LpVariable.dicts("eq1", (range(3*n), range(9, num_v-1)), cat="Binary") #x[i][k]== x[i][k+1]?
#
# for k in range(9, num_v-1):
#     for i in range(3*n):
...
#
#
# u1 = LpVariable.dicts("u1", (range(3*n), range(9, num_v-1)), cat="Binary")  # u1[i][k] = 1 ⇔ все позиции до i-1 равны
# gt1 = LpVariable.dicts("gt1", (range(3*n), range(9, num_v-1)), cat="Binary")  # gt[i][k] = 1 ⇔ x[i][k]=1, x[i][k+1]=0 и до i все равны
# for k in range(9, num_v-1):
#     model += u1[0][k] == 1
...
if model.status == 1:
    ...
    plt.xticks(np.arange(0, num_v, 1), labels=[str(i) for i in range(num_v)])

    ax = plt.gca()
    ax.set_xticks(np.arange(-.5, num_v, 1), minor=True)
    ...
    pos_for_draw = nx.shell_layout(G, shells_for_layout)
        # Альтернативы, если граф не слишком плотный:
    #pos_for_draw = nx.spring_layout(G, seed=42, k=0.5/np.sqrt(G.number_of_nodes()), iterations=30)
...
# 2- 9, 3 - 9, 4 - 9. 5 - 11, 6 - 12, 7 - 12, 8 - 14? 9 -15? 10 - 18?
